...
しかし、マハラノビスゲート閾値としての利用はされており、あまりにもあり得ない位置のボックスはtrackから外れることになる。  
...
max
​
 を提供します。1行目と2行目では、関連付けコスト行列と許容される関連付けの行列を計算します。その後、トラックの年齢
�
nについて反復処理を行い、年齢が増加するトラックに対して線形割り当て問題を解決します。6行目では、過去
�
nフレームで検出と関連付けられていないトラックのサブセット
�
�
T 
n
