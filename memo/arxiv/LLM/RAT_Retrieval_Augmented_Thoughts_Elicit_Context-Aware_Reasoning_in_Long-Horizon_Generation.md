...
#### ベースライン
ベースラインには、元の言語モデル（そのままプロンプトで質問[Direct]）や、𝑛回の検索例を用いたRAGが含まれる。  
...
#### RAGの設定
RATは、RAGを利用する。  
具体的には、コード生成や数学的推論タスクにおいて、codeparrot/github-jupyterデータセットを主要な検索ベクトルライブラリとして使用した。  
Minecraftでの身体的計画タスクには、Minecraft Wiki1とDigMinecraft2ウェブサイトを情報源として利用している。  
オープンエンドの創造的ライティングタスクでは、インターネット上でクエリを検索するためにGoogleを使用する。  
すべての埋め込み計算にはOpenAIのtext-embedding-ada-002 APIサービスを使用した。  

ベンチマーク汚染のリスク（コードライブラリが評価されている問題の正確な解決策を含んでいる可能性のある問題）を認識し、  
Guoらが採用した厳格な前処理方法を適応している。  
ベンチマーク汚染の潜在的な影響と、私たちの前処理戦略の効果については、付録Dで説明する。  

### 3.2. Results
表1に、コード生成の結果を示す。  

表1: コード生成での結果比較。赤文字は、DIRECT(素のLLM利用からの伸び率)  
![図1](../../../data/30.png)

表2に、数学的推論・創造的ライティング・身体的計画タスクの結果を示す。  

表2: RATとDirect(素のLLM利用)の数学的推論・創造的ライティング・身体的計画タスクの結果比較
![図1](../../../data/31.png)

表1と表2から、RATが複数のベンチマークにわたって良い結果であることが分かる。  
特に、コード生成のHumanEvalおよびHumanEval+ベンチマークにおいて、RATはpass@1,5の両方で優れている。  
数学的推論タスクとMinecraftのオープンエンドの身体的計画タスクにおいても、RATは他の全ての方法を大きく上回った。  
これらのタスクは極めて多様であり、RATはすべてのベースラインに対して一貫した改善を達成しています。  

### 3.3. Case Analysis
ここでは、身体的計画タスクと創造的ライティングタスクをケース分析として取り上げる。  
Minecraftのタスクは、複数文書を利用したQAタスクに似て、各タスクの完了にはさまざまなアイテム(文書)の考慮が必要である。  
しかし、インターネット上のMinecraftの情報は断片化されており、タスクの完了は複数の情報源からの情報に依存することになる。  
我々は、ChatGPTのような言語モデルが、ゼロショットCoT推論を通じて必要なアイテムを特定できるが、手順の誤りが発生することを確認した。  
たとえば、ChatGPTは作業台の材料を4つの木のブロックと誤ったことを観測した（正しい答えは4つの板）。  
これはCoTプランの実行可能性の信頼性が低いことを示している。  
RAGでは、質問をクエリとして知識を取得し、最終的なターゲットアイテムに焦点を当てるが、中間のアイテムを不適切に取得し、タスクの改善には貢献しない。  
対照的に、RATは継続的に思考をターゲットの検索で洗練することでCoTの初期回答を改善し、タスクの進行と関連するアイテム知識と密接に一致させる。

オープンエンドの創造的ライティングタスクでは、LLMの生成物の評価は通常、完全性と正確性が利用される。  
例えば「アメリカ南北戦争をタイムラインに沿って要約する」というタスクでは、DIRECTとCoTプロンプトの下でのLLMはしばしば重大な幻覚を生産する。  
例として、「南北戦争は公式には1860年4月12日に南カロライナ州で連合国軍がフォートサムターを攻撃したときに始まった」の回答は間違っている。  
(実際には1861年が正しい)  

このタスクに対するインターネットへの直接的なクエリは、戦争の正確な開始日である1861年4月12日を見落とすこともある。  
さらに、検索から取得したコンテンツを要約する傾向があるRAGアプローチは、RAG-1でもRAG-5でも、このイベントをしばしば見落とす。  
一方、RATは言語モデルのドラフト回答に基づいて検索を行い、通常は特定の日付などの詳細で幻覚が発生することがあるが、  
検索エンジンが「アメリカ南北戦争の開始日」といった関連情報を特定することを妨げない。  
RATは取得したコンテンツを利用してドラフト回答の誤りを特定し、修正するために使用し、単に取得したコンテンツを要約するのではない。  
したがって、RATは推論を通じて完全な生成を達成し、取得した知識を活用することで回答の正確性と信頼性を向上させることができる。  

### 3.4. Ablation Study
#### Ablation on retrieval in RAT
表3にRATとCoT＋RAGの比較の比較を示す。  

表3: RATとCoT＋RAGの比較　　
![図1](../../../data/32.png)  

RATはCoTのタスク自体を修正するのに対し、CoT＋RAGはCoTの各ステップを修正しないことが違いとなる。  
ベースラインであるRAG-1は、質問自体をリトリーバルクエリとして使用する直接的なアプローチを用いている。  
RATは、推論思考の連続的に変更される部分をクエリとして利用することで、より焦点を絞った関連情報の取得をダイナミックに実現する。  

RATはHumanEvalおよびHumanEval+ベンチマークのどちらに対しても、良い結果を示している。  
ここから、RATのCoTのサブタスクの改善と過去の結果を踏まえた生成の有用性が分かる。  

#### Ablation on causal reasoning in RAT
表4に因果的に考えるかそうでないかの結果を示す。  

表4: 因果的に考えるかそうでないかの結果　　
![図1](../../../data/33.png)  

この削除研究では、因果推論と非因果推論のアプローチがRATシステムのパフォーマンスに与える影響を調査する。  
ベースラインは、思考の連鎖（CoT）である。  
表4の結果から、因果推論技術を取り入れることで生成能力が向上していることが分かる。  
非因果メソッドは初期の推論思考を利用して必要な全ステップを直接リトリーブし、最終回答を生成する。  
一方で因果推論は、過去の回答を踏まえて次のタスクに回答をする。  
上記の結果は、因果推論の効果を説明している。  

### 3.5. Robustness of RAT
さらに、異なる言語モデルを用いて、RATをテストした。  
言語モデルは、CodeLlama-7b、ChatGPT、GPT-4を用いた。  
GPT-4で最大の改善が観察され、これは取得したテキストからのインコンテキスト学習の優れた能力に起因していると考えられる。  
MBPP+では、CodeLlama-7bベースのRATが性能が下がっていた。  
この低下は、より小さな言語モデルの限定的なインコンテキスト学習能力が原因である可能性がある。  

数学的推論タスクにおいて、RATはGPT-3.5モデルを使用した場合、全体的な平均で18.44%の顕著な相対的改善を示した。  
この改善の傾向はGPT-4でも持続し、DIRECTからRATへの10.26%の顕著な相対的改善を達成した。  
これらは、RATの堅牢性と、計算的および創造的なタスクにわたる言語モデルのパフォーマンスの効果的な向上を示している。  

## 4. Related Works
### Retrieval-augmented Generation (RAG)
RAGは学習不要であり、LLMが外部世界と交流するためのコスト効率の良い方法である。  
RAGは、コード生成、質問応答、創造的ライティングなどの下流タスクに広く適用されている。  

### Reasoning-enhanced RAG.
最近の研究では、推論を活用してRAGのパフォーマンスを向上させる例もある。  

例えば、以下のようなものがある。  

#### IRCoT
CoTを利用してリトリーバルのためのより良いクエリを生成する。  

#### IRGR
反復的なリトリーバルを行い、マルチホップQAのための適切な前提を探索する。  

#### GEEK
外部知識をクエリするか、長期生成タスクで単一の論理的推論ステップを実行するかを選択できる。  

#### ITRG
最後のステップ生成に基づいてリトリーバルを実行する。  

これらのRAG手法は、質問応答タスクのために単一のクエリを使って外部知識を検索するだけである。  
一方、RATは推論とドラフト回答をオートリグレッシブ方式で使用して検索を行い、  
図2で示すように、様々なタスクでRAGのパフォーマンスを大幅に向上させます。  

### Language Model for Reasoning
言語モデルにおける推論の進展は、CoTの出現以来、様々な方法論が登場し、LLMが自己導出の問題解決戦略を生成する能力を示した。  
これは、最小から最大のプロンプト、ゼロショットCoT、自己一貫性、プロンプトなしのゼロショットCoTなど、様々な応用に発展した。  
基本的なプロンプトを超えて、CreswellらはSelection-Inferenceフレームワークを導入し、Zelikmanらは推論を洗練するためにSTaRを開発した。  
Creswell and Shanahanらは、NyeらのScratchpadのアプローチと似た専用ステップに推論を分割する忠実な推論モデルを提案している。  
Tree-of-ThoughtやGraph-of-Thoughtも、線形CoTではなく複雑な構造に推論パスを拡張している。  
これらの方法は、プロンプトを設計したり、環境からのフィードバックを提供して、より良い計画立案や意思決定を支援し、LLMの推論能力を向上させる。  
一方、RATはRAGを使用して外部知識にアクセスし、その推論プロセスを支援するという点でアプローチが異なる。  

## 5. Conclusion
リトリーバル拡張思考（RAT）を紹介した。  
これは、シンプルでありながら効果的なプロンプト戦略であり、CoTプロンプトとRAGを組み合わせて、困難な長期推論および生成タスクに対処できる。  
私たちの主要なアイデアは、LLMによって生成されたゼロショットの思考の連鎖をクエリとしてRAGを用いて修正し、  
思考を因果関係で修正しながら段階的に応答を生成することである。  
RATは、挑戦的なコード生成、数学的推論、身体的タスク計画、創造的ライティングのタスクにおいて、  
従来のCoTプロンプト、RAG、および他のベースラインよりも顕著な利点を示した。  
